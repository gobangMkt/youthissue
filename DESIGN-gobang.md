# Design System — 고방 (Gobang) Mobile App

1인 가구를 위한 주거생활 플랫폼. 슬로건 "Better alone living".
원룸텔·고시원·쉐어하우스·코리빙·청년주택 검색, 메이트 매칭, 직거래, 맞춤 방 찾기.
모바일 우선, 풀-스크린 섹션 카드, 시안-테일 액션 기반 한국형 UX.
...
토스/당근마켓 계열의 한국형 모바일 UX에 1인 가구 주거 플랫폼의 친근함을 결합.
섹션은 좌우 여백 없이 풀-위드로 붙고, 내부 패딩으로만 여백을 표현한다.
**시안-테일(`#00B2C0`)이 유일한 primary 액션 컬러.** 초록(`#00C73C`) = 성공, 빨강(`#F04452`) = 에러.
퍼플(`#7248F2`)은 Beta·AI 기능 배지 한정으로만 사용.

**Key Characteristics:**
- 회색 페이지 배경 + 흰 섹션 카드 (8px 간격 스택)
- 섹션 카드는 좌우 마진 없이 풀-위드
- 입력 필드: 10px radius, 1.5px border, focus 시 테일 테두리
- 버튼: 10px radius, 전체 너비, 높이 충분(padding 15px)
- 섹션 레이블: 13px 회색 캡션 스타일 (필드명 위 그룹 레이블)
- 필드 레이블: 14px 다크 볼드 (개별 입력 항목 레이블)
- 공지/안내: 테일 tint 박스 (`#E0F8FA` + `#A8E6EC` 테두리)
...
- **Elevated**: `#E0F8FA` — 공지 박스, 요약 카드, 선택된 카드 배경 내부

### Primary & Action
- **Teal**: `#00B2C0` — 주 액션 버튼, 포커스 테두리, 링크, 선택 상태, 탭바 활성
- **Teal Hover**: `#009AAA` — 버튼 hover
- **Teal Dark**: `#1A7A85` — 배너 배경, featured 카드 배경
- **Teal Light BG**: `#E0F8FA` — 키워드 active 배경, 성공 아이콘 배경
- **Teal Selected Card**: `#E8F9FB` — 선택된 카드 배경

### Secondary (Beta·AI 한정)
- **Purple (Beta)**: `#7248F2` — "Beta" 배지, AI 기능 포인트 한정
- **Purple Light**: `#EDE9FF` — Beta 배지 배경, 맞춤 방 찾기 카드 배경
...
- **Border Teal**: `#A8E6EC` — 공지 박스 테두리, 동의 카드 테두리
- **Border Focus**: `#00B2C0` — 포커스/선택 상태
- **Border Valid**: `#00C73C`
- **Border Error**: `#F04452`
...
```
background: #E0F8FA
border: 1.5px solid #A8E6EC
border-radius: 10px
padding: 14px 16px

li: font-size 14px, color #4A5568, line-height 1.6
li::before: '·' 테일색 (#00B2C0), font-weight 700
```
...
modal-icon: 48px circle, #E0F8FA bg
modal-title: 20px/700/#191F28
...
icon: 64px circle, #E0F8FA bg, 28px emoji
...
- 공지/안내는 반드시 `#E0F8FA + #A8E6EC` 테일 tint 박스 사용
- "Beta" 등 신규/AI 기능에만 퍼플(`#7248F2`) 배지 사용
...
- 파란색(`#3182F6`)이나 보라색(`#7248F2`)을 primary 액션에 쓰지 않는다 — 테일(`#00B2C0`)로 통일
...
Teal Hover:      #009AAA
Teal Dark:       #1A7A85
Teal Light BG:   #E0F8FA
Teal Border:     #A8E6EC
Teal Selected:   #E8F9FB
Beta Purple:     #7248F2   (Beta 배지 전용)
