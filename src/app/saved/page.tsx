  ...
  return (
    <main className="max-w-md mx-auto bg-white min-h-screen pb-20">
      <div className="px-4 pt-5 pb-4">
        ...
        <p className="text-sm text-gray-400">저장한 이슈를 모아서 확인하세요</p>
      </div>
      ...
      {savedIssues.length === 0 ? (
        <div className="text-center py-24 px-8">
          ...
          <p className="text-base text-gray-400 mb-6">이슈 상세 페이지에서 "내 혜택함 저장"을 눌러보세요.</p>
          <Link
            href="/"
            className="inline-block bg-indigo-600 text-white text-base font-bold px-6 py-3 rounded-2xl hover:bg-indigo-700 transition-colors"
          >
            오늘의 이슈 보러가기
          </Link>
        </div>
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-medium px-2 py-0.5 rounded-full ${getCategoryColor(issue.category)}`}>
                    {issue.category}
                  </span>
                  <span className="text-sm text-gray-400">TOP {issue.rank}</span>
                </div>
                <button
              ...
              <Link href={`/issue/${issue.id}`}>
                <p className="text-base font-bold text-gray-900 leading-snug mb-2 hover:text-indigo-600 transition-colors">
                  {issue.title}
                </p>
              </Link>
              <p className="text-sm text-gray-500 leading-relaxed line-clamp-2">{issue.summary[0]}</p>
